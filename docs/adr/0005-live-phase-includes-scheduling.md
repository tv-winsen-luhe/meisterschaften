...
site never owns courts or times). The organizer rejected that: after the draw the tournament
...
- After the draw, matches are assigned to a **court** and a **planned start** (day + time). Times
  are explicitly **approximate** — a plan, communicated as "approx.", not a guarantee.
- Matches are scheduled by **bracket position**: a not-yet-decided match shows its feeders ("winner
  M3 vs winner M4") until results fill in the names.
- Each match carries a **live status** (planned → on court X → finished). The operator updates
...
- Because durations are unpredictable, planned times drift during the day. **Resolved:** published
  planned times are static ("approx."); the real-time truth is the match status (on court/finished) on
  the live board, which followers reconcile against. Manual reschedule via the grid stays available for
...
- **Resolved:** the operator places matches by hand on a courts×time grid; the system does not
  ...
  - This is tractable because each member is in exactly one competition (a confirmed invariant —
