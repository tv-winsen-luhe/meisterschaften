# ADR-0024: The Challenger LK cap binds at the draw, not at confirm
...
It cannot be — not at confirm. The LK is **provisional during signup**: it is synced weekly and only
**snapshotted at the draw** (the seeding freeze, ADR-0010), so the LK that "counts" is the one
frozen at the draw, not the value a row happens to carry mid-signup. A hard confirm-time block on a
...
The Challenger cap **binds at the draw, on the frozen LK** — the single moment the LK is
authoritative. Confirming a registration during signup is a provisional act, so a too-strong entry
...
- The hard eligibility check is **draw-time work**, built with the draw (which does not exist
  ...
  provisional seeding list, which surfaces every LK for the operator to eyeball before drawing.
- `CHALLENGER_MIN_LK` becomes an operator-tunable lever at the draw (today a fixed shared constant);
  making it adjustable is part of the draw work, not before.
