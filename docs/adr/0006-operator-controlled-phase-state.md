...
- Status: accepted; **revised by ADR-0027** (the four phases collapse to three — `signup` →
  `tournament` → `post-event`; the `draw`/`live` distinction becomes a per-competition-derived
...
The site presents itself differently across the four phases (signup, draw, live, post-event):
...
`live` / `post-event` — English identifiers; German display names are surfaced as user-facing copy
...
- **signup → draw** closes registration and **freezes the seeding LK**. Mechanism (refined by
  ...
  signup, so it is simply a no-op afterward; no suppression flag is needed. Before the draw, LKs
  keep updating and the provisional seeding list reflects them live.
- **live → post-event** freezes results — the brackets and the schedule become read-only.

## Consequences

- Phase transitions are deliberate acts, not clock events — the draw begins when the operator
  starts it, live begins when brackets are published. This matches how the day is actually run.
- A small settings/state record in D1 becomes the master switch every public surface keys off.
- The LK sync must respect the freeze: after the draw, seeding LK is immutable even though nuLiga
  data keeps updating.
