...
// The schedule publication seams (ADR-0041), kept out of the admin shell like useReveal: the published
// flag and the publish/reset actions. The initial flag is read once on mount from the lightweight
// operator endpoint; each action sets it **deterministically** from its own outcome (publish ⇒ true,
// reset ⇒ false) rather than re-reading, so a flaky follow-up GET can never leave the control showing the
// wrong state after a write that did take.

type Client = ReturnType<typeof hc<AppType>>
// The shell's shared mutation wrapper (401-regate + error/success toast + reload), passed in so publish and
// reset share its behaviour and each fire exactly one confirmation toast.
type Mutate = (run: () => Promise<Response>, success: string) => Promise<boolean>
...
export const useSchedule = (client: Client, mutate: Mutate): ScheduleApi => {
  ...
  return { published, publishSchedule, resetSchedule }
}
