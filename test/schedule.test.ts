...
describe('suggestSchedule', () => {
  // A schedulable match for the suggest tests (same shape as the validatePlacement helper `pm`).
  const sm = (
    id: number,
    round: number,
    position: number,
    opts: { p?: [number | null, number | null]; at?: Placement; outcome?: string | null } = {}
  ): SchedulableMatch => ({
    id,
    competition: 'mens',
    bracket: 'main',
