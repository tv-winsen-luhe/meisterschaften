...
// Konkurrenzen (slug ∈ COMPETITION_SLUGS), with no separate `selectable` flag to drift (ADR-0022).
...
describe('signupCompetitions', () => {
  ...
  it('offers only Konkurrenzen the register contract accepts', () => {
  ...
  // Konkurrenz from the form with no type error (slug is string-typed). Set equality catches both a
  // dropped slug and a contract slug that has no catalogue entry — the other half of "drift-proof".
  it('offers exactly the registerable Konkurrenzen, no more, no fewer', () => {
